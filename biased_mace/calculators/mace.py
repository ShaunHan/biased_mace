...
from biased_mace.tools.compile import prepare
from biased_mace.tools.scripts_utils import extract_model
...
class MACECalculator(Calculator):
        ...
        **kwargs,
        ...
        if self.bias_weight > 0.0:
            ...
            if not hasattr(self.models[0], "global_readout"):
                raise ValueError(
                    "use_global_readout=True is required when bias_weight > 0."
                )
    ...
    def _clone_batch(self, batch, positions_requires_grad=False):
        batch_clone = batch.clone()
        if self.use_compile:
            batch_clone["node_attrs"].requires_grad_(True)
            batch_clone["positions"].requires_grad_(True)
        elif positions_requires_grad:
            batch_clone["positions"].requires_grad_(True)
        return batch_clone

    def _make_global_mask(self, n_atoms: int):
        if self.bias_indices is None:
            return None
        mask = torch.zeros(n_atoms, dtype=torch.bool, device=self.device)
        idx = torch.as_tensor(self.bias_indices, dtype=torch.long, device=self.device)
        if torch.any(idx < 0) or torch.any(idx >= n_atoms):
            raise IndexError("bias_indices out of range for the current Atoms object.")
        mask[idx] = True
        return mask

    def _compute_global_descriptor(self, atoms):
        batch = self._atoms_to_batch(atoms)
        batch = self._clone_batch(batch, positions_requires_grad=True)
        mask = self._make_global_mask(len(atoms))
        out = self.models[0](
            batch.to_dict(),
            compute_force=False,
            ...
            global_descriptor_mask=mask,
            training=False,
        )
        if out.get("global_descriptor", None) is None:
            raise ValueError("The loaded model does not return a global descriptor.")
        return out["global_descriptor"].squeeze(0)
    ...
    def get_descriptors(self, atoms=None, invariants_only=True, num_layers=-1,
                        global_descriptor=False, bias_indices=None):
